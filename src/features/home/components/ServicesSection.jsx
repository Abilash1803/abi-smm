  ...
  return (
    <div className="relative overflow-hidden pb-12 sm:pb-20 bg-gradient-to-br from-purple-50/30 via-white/50 to-purple-100/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        {/* CTA and Services Header - Mobile Optimized */}
        <div className="max-w-6xl mx-auto px-4 sm:px-6 text-center mb-8 sm:mb-20">
          ...
          <div className="bg-white/40 backdrop-blur-xl rounded-xl sm:rounded-3xl border border-white/50 p-4 sm:p-8 md:p-12 shadow-2xl">
            ...
            <button 
              ...
              className="px-6 sm:px-10 py-3 sm:py-4 bg-gradient-to-r from-[#FF6B35] to-[#FFA500] text-white rounded-full font-semibold text-sm sm:text-lg hover:shadow-lg hover:shadow-[#FF6B35]/40 transition-all hover:scale-105 w-full sm:w-auto"
            >
        ...
        {/* Platform Cards - All 4 cards in row on mobile, scaled to fit */}
            ...
            {platformsData.map((platform) => (
              <div 
                ...
                onClick={() => handlePlatformClick(platform.path)}
                  ...
                  shadow-2xl w-full max-w-[260px] aspect-[9/16] flex flex-col relative hover:scale-105 transition-all duration-300 
                  cursor-pointer group overflow-hidden`}
                >
                
                {/* Phone-style notch - Scaled for mobile */}
                ...
                {/* Screen content area filling the entire phone */}
                <div className="flex flex-col h-full pt-8 sm:pt-10 lg:pt-14 px-4 sm:px-3 lg:px-4 bg-white/95 rounded-[1rem] sm:rounded-[2.5rem] lg:rounded-[2.7rem] shadow-inner">
                  ...
                  {/* Platform description - Better spacing for mobile/tablet */}
                  ...
                  <div className="space-y-2 sm:space-y-3 lg:space-y-4 mt-auto mb-4 sm:mb-5 lg:mb-6">
                    {platform.buttons.map((btn, i) => (
                      <button 
                        key={i} 
                        className={`w-full px-3 sm:px-3 lg:px-4 py-2 sm:py-2.5 lg:py-2.5 ${platform.btnClass} text-white rounded-lg sm:rounded-lg lg:rounded-xl text-xs sm:text-xs lg:text-sm font-semibold transition-all shadow-lg hover:scale-105 group-hover:shadow-xl border border-white/20`}
                        onClick={(e) => {
                          e.stopPropagation()
